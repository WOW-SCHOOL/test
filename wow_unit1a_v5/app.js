...
/* audio engine: live UK TTS. No browser speechSynthesis. */
let currentAudio=null,playToken=0;
function providerUrls(text,voice='Brian'){
 const q=encodeURIComponent(text);
 return [
  `https://translate.google.com/translate_tts?ie=UTF-8&client=tw-ob&tl=en-GB&q=${q}`,
  `https://api.streamelements.com/kappa/v2/speech?voice=${encodeURIComponent(voice)}&text=${q}`
 ];
}
function stopAudio(){playToken++;if(currentAudio){try{currentAudio.pause();currentAudio.src=''}catch(e){}currentAudio=null}}
function playOne(text,voice='Brian',onState=()=>{}){
 return new Promise(async(resolve,reject)=>{
  const token=playToken;
  for(const url of providerUrls(text,voice)){
   if(token!==playToken)return reject(new Error('cancelled'));
   try{
    await new Promise((res,rej)=>{
     const a=new Audio();currentAudio=a;a.preload='auto';
     let settled=false;const timer=setTimeout(()=>{if(!settled){settled=true;a.src='';rej(new Error('timeout'))}},3500);
     a.oncanplay=async()=>{if(settled)return;settled=true;clearTimeout(timer);try{onState('playing');await a.play()}catch(e){return rej(e)}};
     a.onended=()=>{currentAudio=null;res()};a.onerror=()=>{if(!settled){settled=true;clearTimeout(timer)};rej(new Error('audio error'))};a.src=url;
    });
    return resolve();
   }catch(e){}
  }
  reject(new Error('No UK voice provider available'));
 })
}
async function playText(text,voice='Brian',btn){
 stopAudio();playToken++;const token=playToken;btn?.classList.add('busy');btn&&(btn.textContent='■ Стоп');
 try{await playOne(text,voice,()=>{});if(token===playToken){btn?.classList.remove('busy');btn&&(btn.textContent='▶ Прослушать')}}
 catch(e){if(token===playToken){btn?.classList.remove('busy');btn?.classList.add('error');btn&&(btn.textContent='Аудио недоступно');setTimeout(()=>{btn?.classList.remove('error');btn&&(btn.textContent='▶ Прослушать')},2200)}}
}
async function playScript(lines,btn){stopAudio();playToken++;const token=playToken;btn?.classList.add('busy');btn&&(btn.textContent='■ Стоп');try{for(const l of lines){if(token!==playToken)break;await playOne(l.text,l.voice)}if(token===playToken){btn?.classList.remove('busy');btn&&(btn.textContent='▶ Прослушать')}}catch(e){if(token===playToken){btn?.classList.remove('busy');btn?.classList.add('error');btn&&(btn.textContent='Аудио недоступно');setTimeout(()=>{btn?.classList.remove('error');btn&&(btn.textContent='▶ Прослушать')},2200)}}}
